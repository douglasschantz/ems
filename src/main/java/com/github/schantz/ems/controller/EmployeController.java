...
public class EmployeController {
	...
	// build update employes REST APT
	@PutMapping("{id}")
	public ResponseEntity<EmployeDto> updateEmploye(@PathVariable("id") Long employeId,
			@RequestBody EmployeDto updateEmploye) {
		EmployeDto employeDto = employeService.updateEmploye(employeId, updateEmploye);
		return ResponseEntity.ok(employeDto);
	}
	...
	public ResponseEntity<String> deleteEmploye(@PathVariable("id") Long employeId){
		...
		return ResponseEntity.ok("Employe deleted sucessfull!.");
	}
